...
2. Plug the ZMPT101B/SCT013 to an ADC_PIN. This voltage should be close to the voltage you will eventually read during the operation of this device. Do not connect to AC sources that generate square waves (no-break, inverters, etc...).
3. Edit *verify-adc-pin.ino* and change the GPIO pin according to your hardware.
...
void loop()
{
  int value_raw = analogRead(ADC_PIN);
  float voltage = (value_raw * 3.3) / 4095.0;
